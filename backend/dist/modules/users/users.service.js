...
let UsersService = class UsersService {
    ...
    async findAll(page = 1, limit = 50, search) {
        ...
        if (search) {
            query.andWhere('user.firstName ILIKE :search OR user.lastName ILIKE :search OR user.email ILIKE :search OR user.username ILIKE :search', { search: `%${search}%` });
        }
    ...
    async updatePassword(id, updatePasswordDto) {
        ...
        if (!user)
            throw new common_1.NotFoundException('Usuario no encontrado');
        const passwordMatch = await bcrypt.compare(updatePasswordDto.currentPassword, user.passwordHash);
        if (!passwordMatch)
            throw new common_1.BadRequestException('La contraseña actual es incorrecta');
        if (updatePasswordDto.newPassword !== updatePasswordDto.confirmPassword) {
            throw new common_1.BadRequestException('Las contraseñas nuevas no coinciden');
        }
